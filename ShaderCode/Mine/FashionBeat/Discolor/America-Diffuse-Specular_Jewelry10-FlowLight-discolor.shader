Shader "America/Character/America-Diffuse-Specular_Jewelry10-FlowLight-discolor"
{
	Properties
	{
		...
		_MaskFlowSpeed ("Mask Flow Speed", Vector) = (1, 0, 0, 0)
	...
	SubShader 
	{
		...
		Pass
		{
			...
			float4 _FlowSpeed;
			float4 _MaskFlowSpeed;
			float4 _FlowLightNoise_ST;
			float4 _FlowLightMask_ST;
			...
			v2f vert(appdata v)
			{
				...
				o.uvMask.xy = v.uv * _FlowLightMask_ST.xy + _Time.yy * _MaskFlowSpeed.xy;
			...
			fixed4 frag(v2f i) : SV_TARGET
			{
				...
				// flow light
				fixed4 maskTex = tex2D(_FlowLightMask, i.uvMask.xy);
