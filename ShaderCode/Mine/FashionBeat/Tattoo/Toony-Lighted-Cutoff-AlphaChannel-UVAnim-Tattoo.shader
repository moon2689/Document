Shader "America/Character/Tattoo/NewEffect/Toony-Lighted-Cutoff-AlphaChannel-UVAnim"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_TARGET
			{
				...
				fixed delta = 1 / _SizeXY;
				int cellx = fmod(_Index, _SizeXY);
				int celly = _Index / _SizeXY;
				float2 albedoUV = float2(cellx * delta, celly * delta + delta) + float2(i.uv.x * delta, i.uv.y * delta);
				fixed4 albedo = tex2D(_MainTex, albedoUV);
