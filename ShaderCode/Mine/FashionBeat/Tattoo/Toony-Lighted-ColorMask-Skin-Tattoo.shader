Shader "America/Character/Tattoo/NewEffect/Toony-Lighted-ColorMask-Skin"
{
	Properties
	{
	...
	SubShader 
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_TARGET
			{
				...
				fixed mask = tex2D(_MainTexMask, i.uv).r;
				bool isSkin = mask < 0.1;

				// 纹身
				if(isSkin)
				{
					mainTex = ApplyTattoo(i.uv, _TattooTex1, _TattooPos1, mainTex);
					mainTex = ApplyTattoo(i.uv, _TattooTex2, _TattooPos2, mainTex);
					mainTex = ApplyTattoo(i.uv, _TattooTex3, _TattooPos3, mainTex);
					mainTex = ApplyTattoo(i.uv, _TattooTex4, _TattooPos4, mainTex);
				}
				...
				fixed4 col = fixed4(diffuse * _DiffuseRate, mainTex.a);
