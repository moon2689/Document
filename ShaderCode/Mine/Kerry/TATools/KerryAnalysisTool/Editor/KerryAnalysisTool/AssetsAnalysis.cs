...
public class AssetAnalysisWindow : EditorWindow
{
	...
	private float winHeightHalf = 0f;
	...
	private void FindReference(Object resource)
	{
		...
		EditorGUIUtility.ExitGUI();//修复异常bug:https://answers.unity.com/questions/1353442/editorutilitysavefilepane-and-beginhorizontal-caus.html
	}
	private void DoAssetAnalyse(string path)
	{
		...
		if (_prefab != null)
		{
			...
			foreach (var obj in dependencies)
			{
				...
				if (tex)
				{
					int memSize = UnityEngine.Profiling.Profiler.GetRuntimeMemorySize(tex) / 2;
					sumSize += memSize;
