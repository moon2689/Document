	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag (v2f i) : SV_Target
			{
				...
				half4 base_color = pow(albedo_color_gamma, 2.2) * _BaseColor;
				...
				half half_lambert = (max(0.0, dot(normal_dir, light_dir)) + 1.0) * 0.5;
				#ifdef _DIFFUSECHECK_ON
				//half3 direct_diffuse = half_lambert * base_color * atten * _LightColor0.xyz;
				...
				float3 final_spec1 = spec_term1 * aniso_atten * spec_color1 * _LightColor0.rgb;
				//spec2
				...
				float3 final_spec2 = spec_term2 * aniso_atten * spec_color2 * _LightColor0.rgb;
				...
				float3 final_color = direct_diffuse * 2.0 + direct_specular + env_specular;
