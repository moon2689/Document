	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag (v2f i) : SV_Target
			{
				...
				half ao = lerp(1.0, aoCol.r, _AOAdjust);
				half3 view_dir = normalize(_WorldSpaceCameraPos.xyz - i.pos_world);
				half3 reflect_dir = reflect(-view_dir, normal_dir);

				reflect_dir = RotateAround(_Rotate, reflect_dir);
				
				half4 roughnessCol = SAMPLE_TEXTURE2D(_RoughnessMap, sampler_RoughnessMap, i.uv);
				float roughness = roughnessCol.r;
				roughness = saturate(pow(abs(roughness), _RoughnessContrast) * _RoughnessBrightness);
				...
				//half3 final_color_linear = pow(abs(final_color), 2.2);
