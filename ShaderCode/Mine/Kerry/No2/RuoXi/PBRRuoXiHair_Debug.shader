Shader "RuoXi/URP/Lit/PBR/PBRRuoXiHair_Debug"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float _CutOff, _Roughness, _Specular, _Scatter;
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                clip(alphaMap - _CutOff);

                float3 newWorldNormal = mul(float3(0,1,0), TBN);

                float3 diffuseCol = baseMap.rgb;
                float3 specCol = _Specular * 0.04.xxx;
                
                float3 V = GetWorldSpaceNormalizeViewDir(worldPos);
                float3 dirLighting, indirLighting;
                DirectLighting_float(diffuseCol, specCol, _Roughness, worldPos, newWorldNormal, V, _Scatter, dirLighting);
                IndirectLighting_float(diffuseCol, specCol, _Roughness, newWorldNormal, V, _Scatter, indirLighting);
                float3 pbrLighing = dirLighting + indirLighting;
                //debugCol = specCol;
                //pbrLighing = debugCol;
