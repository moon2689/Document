Shader "Kerry/URP/PBR/ASE_Eye 1"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag (VertexOutput IN) : SV_Target
			{
				...
				// 虹膜颜色 sclera
				float2 eyeBallUV;
				ScaleUVsByCenter_float(IN.uv, _ScalebyCenter, eyeBallUV);
				float4 scleraCol = SAMPLE_TEXTURE2D(_ScleraMap, sampler_ScleraMap, eyeBallUV) * _ScleraBrightness;
			
				// 巩膜深度
				float4 heightMap = SAMPLE_TEXTURE2D(_MidPlaneHeightMap, sampler_MidPlaneHeightMap, IN.uv);
				float2 uv_heightMap2 = float2(_ScalebyCenter * _IrisRadius + 0.5, 0.5);
				float4 heightMap2 = SAMPLE_TEXTURE2D(_MidPlaneHeightMap, sampler_MidPlaneHeightMap, uv_heightMap2);
				float irisDepth = max(heightMap.r - heightMap2.r, 0.0) * _IrisDepthScale;
				...
				float2 irisUV;
				float irisConcavity;
				EyeRefraction_float(eyeBallUV, worldNormal, worldView, _IOR, _IrisRadius, irisDepth, worldEyeDir, worldTangent, irisUV, irisConcavity);
				...
				// 巩膜颜色
				float4 irisCol = SAMPLE_TEXTURE2D(_IrisColorMap, sampler_IrisColorMap, uv_Scale) * _IrisBrightness * limbuxParam;

				float smoothParam = 1.0 - (distance(eyeBallUV, float2(0.5,0.5)) - _IrisRadius + 0.045) / 0.045;
				...
				// 法线
				float2 uvScale;
				ScaleUVsByCenter_float(IN.uv, _SceleraNormalUVScale, uvScale);
				float3 sceleraNormalMap = UnpackNormalScale(SAMPLE_TEXTURE2D(_SceleraNormalMap, sampler_SceleraNormalMap, uvScale), _SceleraNormalStrength);
				sceleraNormalMap.z = lerp(1, sceleraNormalMap.z, saturate(_SceleraNormalStrength));
				float3 surfaceNormal = lerp(sceleraNormalMap, float3(0,0,1), irisMask);
				float3 worldNormal1 = normalize(mul(surfaceNormal, TBN));

				float2 uv_scale3;
				ScaleUVFromCircle_float(irisUV, _IrisNormalUVScale, uv_scale3);
				float3 irisNormalMap = UnpackNormalScale(SAMPLE_TEXTURE2D(_IrisNormalMap, sampler_IrisNormalMap, uv_scale3), _IrisNormalStrength);
				irisNormalMap.z = lerp(1, irisNormalMap.z, saturate(_IrisNormalStrength));
				float3 irisNormal = normalize(mul(BlendNormal(irisNormalMap, tangentEyeDir), TBN));
