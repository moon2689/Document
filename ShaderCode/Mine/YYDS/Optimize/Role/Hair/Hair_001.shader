Shader "TA/Optimize/Role/Hair/Hair_001"
{
	Properties
	{
		...
		_EdgeBrightness("EdgeBrightness", Float) = 3
	}
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float3 Color = ( (-0.015 + (_MainLightColor.a - 0.0) * (_EdgeBrightness - -0.015) / (1.0 - 0.0)) * Color362 ).rgb;
		...
		Pass
        {
            ...
            {
				half4 alpha = SAMPLE_TEXTURE2D(_Diffuse, sampler_Diffuse, input.uv);
				clip(alpha - 0.1);
                return 0;
            }
