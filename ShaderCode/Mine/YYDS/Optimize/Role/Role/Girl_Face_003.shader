Shader "TA/Optimize/Role/Role/Girl_Face_003"
{
	Properties
	{
		...
		_BlushDiffuse("BlushDiffuse", 2D) = "white" {}
		_BlushColor("BlushColor", Color) = (1,1,1,1)
		_BlushDiffuseStrength("BlushDiffuseStrength", Float) = 1
		_BlushStrength("BlushStrength", Float) = 1
	}
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN ) : SV_Target
			{
				...
				float3 worldNormal78 = UnpackNormalScale( tex2D( _Normal, uv_Normal ), 1.0f );
				...
				float2 texCoord386 = IN.ase_texcoord8.xy * float2( 1,1 ) + float2( 0,0 );
				float2 appendResult385 = (float2(( ( texCoord386.x * 0.5113 ) + -0.1265 ) , ( ( texCoord386.y * 1.025 ) + -0.4485 )));
				float4 tex2DNode398 = tex2D( _BlushDiffuse, ( appendResult385 * 3.9 ) );
				float4 Blush393 = ( tex2DNode398 * _BlushColor * _BlushDiffuseStrength );
				float Blush_A391 = ( tex2DNode398.a * _BlushStrength );
				float4 lerpResult403 = lerp( ( ( DarkSideColor401 * lerpResult369 * _BaseColor ) + Specilar276 ) , Blush393 , Blush_A391);
				
				float MaskMap_S353 = tex2DNode351.r;
				
				float3 Albedo = lerpResult403.rgb;
