#include "BRDF_Common.hlsl"

half ClampSpecRoughness(half roughness)
{
    return roughness;
    //return clamp(roughness, 0.3, 1); //太过光滑，过了Bloom会导致过闪
}

half3 BRDF_HarfLambertLighting(Light light, half3 diffCol, half3 N, half3 V, half halfLambertIntensity, inout half3 radiance, inout half halfLambert)
{
...
half3 BRDF_HarfLambertLighting_FixN(Light light, half3 diffCol, half3 N, half3 V, half halfLambertIntensity, inout half3 radiance, inout half halfLambert)
{
...
half3 BRDF_StandardSpecularLighting(half3 specCol, half3 N, half3 V, half3 L, half NoV, half NoL, half roughness, half3 radiance)
{
    ...
    //roughness = ClampSpecRoughness(roughness); //太过光滑，过了Bloom会导致过闪
...
half3 DirectLighting_Standard(Light light, half3 diffCol, half3 specCol, float3 worldPos, half3 N, half3 V, half NoV, half roughness)
{
    ...
    // 漫反射
    half3 diffuseLighting = BRDF_HarfLambertLighting(light, diffCol, N, V, 0.5, radiance, NoL);
...
half3 IndirectLighting_Standard(half3 diffCol, half3 specCol, float3 worldPos, half3 N, half3 V, half NoV, half roughness)
{
    // SH
    half3 sh = SampleSH(N);
...
half3 Diffuse_FixN(Light light, half3 diffCol, half3 N, half3 V)
{
    half3 L = V;//light.direction;
    half NoL = dot(N, L);
...
half3 Diffuse_FixN(Light light, half3 diffCol, half3 N, half3 V, inout half halfLambert)
{
    half3 L = V;//light.direction;
    half NoL = dot(N, L);
