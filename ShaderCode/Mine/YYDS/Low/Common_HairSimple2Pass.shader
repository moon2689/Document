Shader "TA/Role/Low/Common_HairSimple2Pass"
{
    Properties
    {
        // 纹理------------------------------------------------------------>
		...
		_Roughness("粗糙度", Range(0,1)) = 0.3

        [Header(Env)]
        [Toggle_Switch] _Env("是否有环境光", float) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 Fragment(Varyings input) : SV_Target
            {
                ...
                half3 baseColor = baseMap.rgb * _BaseColor.rgb * _BaseColor.a * _DiffuseIntensity;
                half alpha = baseMap.a;
                //return baseColor.xyzz * maskHair;
                ...
                half halfLambert;
                    ...
                    diffuseLighting = Diffuse_FixN(diffCol, N, V, halfLambert);
                #else
                    diffuseLighting = HalfLambert_Diffuse(diffCol, N, V, halfLambert);
                #endif
        ...
        Pass
        {
            ...
            half4 Fragment(Varyings input) : SV_Target
            {
                ...
                // 根据世界坐标位置剪裁
                #if _CLIPBYWORLDPOS_ON
                    ClipRole(input.positionWS.xyz, _ClipWorldPosParam.xyz, _ClipWorldPosParam.w);
                #endif
				return half4(baseMap.rgb * _BaseColor.rgb, 1);
            }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 Fragment(Varyings input) : SV_Target
            {
                ...
                #if _FIXERRORNORMAL_ON
                    diffuseLighting = diffCol * (abs(dot(N, V)) * 0.7 + 0.3);
                #else
                    diffuseLighting = diffCol * (dot(N, V) * 0.7 + 0.3);
                #endif
                finalColor += diffuseLighting;
